...
class ChildTfBroadcaster3(Node):
    def __init__(self):
        super().__init__('child_node3')
        self.get_logger().info("Child TF Broadcaster3 노드 시작")
    ...
    def timer_callback(self):
        ...
        #6. 부모/자식 프레임 지정
        tf_stamped.header.frame_id = 'base_link'  # 부모 프레임을 'base_link'로 변경
        tf_stamped.child_frame_id = 'child1_link'
        ...
        #8. 쿼터니언 구하기
        #8.1 반지름 1.0m인 원형 궤적(x=1.0*cos(angle), y=1.0*sin(angle))
        r = 1.0
